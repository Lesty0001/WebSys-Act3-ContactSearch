function searchContacts() {
    ...
    fetch('https://lesty0001.github.io/WebSys-Finals-Ajax/data.json')
        .then(response => {
            if (!response.ok) {
    ...
    contacts.forEach(contact => {
        html += `
            <tr>
                ...
                <td>
                    ${contact.email ? `<div><i class="fas fa-envelope me-2 text-success"></i> ${contact.email}</div>` : ''}
                ...
                <td>
                    ${contact.company ? `
                    <div class="company-label"><i class="fas fa-building me-2 text-warning"></i> ${contact.company.name || 'N/A'}</div>
